...
def filter_data(input_file: str, output_file: str):
    ...
    for item in tqdm(data):
        ...
        for candidate in candidates:
            _, answer_dict, reasoning_trace = extract(candidate, raw_answer=True)
        ...
        if len(reasoning_traces) > 0:
            ...
            assistant_response = f"<think>{chosen_trace}</think> Request: {item['prompt_harm_label']} \nCompletion: {item['response_refusal_label']} \nResponse: {item['response_harm_label']} "
            conversations = [
            ...
            filtered_items.append({
                ...
                "response_refusal_label": item['response_refusal_label'],
                "response_harm_label": item['response_harm_label'],
                "golden_reasoning_trace": item['golden_reasoning_trace'],
            })
    ...
    logger.stat("data split distribution")
...
if __name__ == "__main__":
    input_file = "experiment-4/temp/guardreasoner-generate.jsonl"
    output_file = "experiment-4/temp/guardreasoner-filtered.jsonl"
    filter_data(
